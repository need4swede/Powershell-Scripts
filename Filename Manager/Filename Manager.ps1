    ...
    while($loop)
    {
        ...
        if ($browse.ShowDialog() -eq "OK")
        {
            ...
            function Define-Task {
                ...
                while($task_loop)
                {
                    ...
                    if ($task_type -eq "1")
                    {
                        ...
                        function add-prefix {
                            function list-types {
                                write-host ""
                                Get-ChildItem -Recurse | Group-Object psparentpath, extension | ? {(($_.Name -split ", .")[1]) -ne $Null } | ft -AutoSize @{Expression={($_.Name -split ", .")[1]};Label="File Types within this folder"}
                            } list-types
              ...
              clear-host
              write-host ""
              write-host "Filenames Modified Successfully"
              write-host ""
        }
